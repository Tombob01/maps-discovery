      ...
      .then(({ runId }) => {
        ...
        return facade.executeRun({
          ...
          function onComplete(run: import('../types/ui').Run) {
            ...
            facade.listRecords(runId).then(records => {
          ...
          intervalId = setInterval(() => {
            if (Date.now() - startedAt > MAX_POLL_MS) {
              stopPolling();
              addEvent('Run is taking longer than expected.', 'warn');
              dispatch({ type: 'SET_RUN_STATUS', status: 'failed' });
              dispatch({ type: 'SET_ERROR', error: 'Run is taking longer than expected.' });
              return;
            }
            facade.getRun(runId).then(run => {
              dispatch({ type: 'SET_STATS', stats: run.stats });
              ...
              else if (run.status === 'failed') { onFailed(run); }
