...
const booleanString = zEnum([
  ...
  "yes",
  "no",
] as const).transform((v: string) => v === "true" || v === "1" || v === "yes");
...
const EnvSchema = zObject({
  ...
  QUERY_ENGINE_DEFAULT_STRATEGIES: nonEmptyString.default(
...
// ---------------------------------------------------------------------------
// Structured config object built from the flat parsed env
// ---------------------------------------------------------------------------
...
function buildConfig(parsed: ParsedEnv): AppConfig {
  ...
  return {
    ...
    queryEngine: {
      ...
      defaultStrategyIds: Object.freeze(
        parsed.QUERY_ENGINE_DEFAULT_STRATEGIES.split(",")
          .map((s) => s.trim())
          .filter((s) => s.length > 0),
      ),
    },
