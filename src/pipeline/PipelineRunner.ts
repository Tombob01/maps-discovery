...
import type { IPipelineStage, StageContext } from "./IPipelineStage.js";
...
export class PipelineRunner<T> {
  ...
  private readonly opts: Required<PipelineRunnerOptions>;
  ...
  constructor(
    ...
    private readonly stage: IPipelineStage<T>,
  ...
  async start(): Promise<void> {
    ...
    while (this.running) {
      ...
      if (job === null) {
        emptyPolls++;
        if (emptyPolls >= this.opts.maxEmptyPolls) break;
        await this.sleep(this.opts.pollIntervalMs);
        continue;
      }
  ...
  private sleep(ms: number): Promise<void> {
    return new Promise<void>((resolve) => {
      ...
      const fn = Function("cb", "ms", "return setTimeout(cb, ms)") as (
        cb: () => void,
        ms: number,
      ) => void;
      fn(resolve, ms);
