...
export class QueryEngine {
  ...
  constructor(
    ...
    private readonly expander: IQueryExpander,
    private readonly geoResolver: IGeoResolver,
    private readonly config: QueryEngineConfig,
    private readonly niches?: NicheDictionaryIndex,
  ) {
  ...
  async generate(
    ...
    const seenHashes = new Map<QueryHash, QueryID>();

    for (const providerId of providerIds) {
      const providerResult = await this._generateForProvider(
        seed,
        runId,
        providerId,
        seenHashes,
  ...
  private async _generateForProvider(
    seed: QuerySeed,
    runId: RunID,
    providerId: string,
    seenHashes: Map<QueryHash, QueryID>,
  ): Promise<Result<readonly GeneratedQuery[], QueryEngineErrorDetail>> {
    // 1. Build root query
    ...
    const result: GeneratedQuery[] = [canonRoot];

    // 3. Expand — skip if no strategy IDs requested
    const strategyIds =
      seed.expansionStrategyIds ?? this.config.expansion.defaultStrategyIds;
    if (strategyIds.length === 0) return ok(result);
    ...
    const maxVariants =
      seed.maxVariants ?? this.config.expansion.defaultMaxVariants;
    const budget = maxVariants - 1; // root occupies slot 0
    if (budget <= 0) return ok(result);
    ...
    for (const variant of expandResult.value) {
      ...
      if (!vCanon.ok) continue;

      const cv = vCanon.value;
      if (cv.isDuplicateOfExisting) continue;
