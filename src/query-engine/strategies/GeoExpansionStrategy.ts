...
export class GeoExpansionStrategy extends BaseExpansionStrategy {
  override readonly id = "geo" as const;
  override readonly description =
    "Substitutes city/state with sub-locations from geo dictionary";

  private static readonly CONFIDENCE = 0.75;

  constructor(private readonly geodict: GeoDictionaryIndex) {
    super();
  }
  ...
  protected override _apply(
    ...
    if (!textLower.includes(targetLower)) return [];
    ...
    for (const sub of subLocations) {
      ...
      const variant = replaceLocationCaseInsensitive(
        query.rawText,
        replaceTarget,
        sub.name,
      );
      if (variant === query.rawText) continue;
      ...
      for (const alias of sub.aliases) {
        if (candidates.length >= context.maxNew) break;
        const aliasVariant = replaceLocationCaseInsensitive(
          query.rawText,
          replaceTarget,
          alias,
        );
        ...
        const isPrimaryName =
          sub.name.toLowerCase() === aliasLower ||
          aliasLower.includes(sub.name.toLowerCase());
        if (isPrimaryName) continue;
 ...
 */
function extractCityFromDisplayName(displayName: string): string | undefined {
...
function replaceLocationCaseInsensitive(
  text: string,
  target: string,
  replacement: string,
): string {
