...
// ---------------------------------------------------------------------------
// IStageCheckpointStore — persistence for stage resumability
// ---------------------------------------------------------------------------
...
export interface IStageCheckpointStore {
   ...
   */
  upsert(
    checkpoint: Omit<StageCheckpoint, "id" | "createdAt">,
  ): Promise<Result<StageCheckpoint>>;
  ...
  get(
    ...
    stage: StageName,
    entityId: string,
  ): Promise<Result<StageCheckpoint | null>>;
  ...
  ): Promise<Result<readonly StageCheckpoint[]>>;
}
